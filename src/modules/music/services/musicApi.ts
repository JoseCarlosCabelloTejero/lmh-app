...
// Usar React Query (TanStack Query)
...
export const uploadLyricsApi = async (year: number, file: File): Promise<ApiFile> => {
  console.log(`[API] Subiendo letra ${file.name} para ${year}...`);
  return new Promise(res => setTimeout(() => res(
    { id: `m-${Math.random()}`, name: file.name, type: file.type.includes('pdf') ? 'pdf' : 'jpg' }
  ), 1500));
};
...
export const deleteLyricsApi = async (year: number, id: string): Promise<void> => {
  console.log(`[API] Borrando letra ${id} para ${year}...`);
  return new Promise(res => setTimeout(res, 500));
};
